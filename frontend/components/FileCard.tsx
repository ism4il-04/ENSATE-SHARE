...
export function FileCard({ file, variant = 'default', animationDelay = 0 }: FileCardProps) {
    ...
    const viewerHref = `/viewer?id=${encodeURIComponent(file._id)}&url=${encodeURIComponent(
        file.fileUrl
    )}&name=${encodeURIComponent(file.displayName || file.fileName)}&type=${encodeURIComponent(file.fileType)}`;

    const apiBase = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api';
    const downloadHref = `${apiBase}/files/${file._id}/download`;

    return (
        <Link href={viewerHref} className="block focus:outline-none" title={file.displayName || file.fileName}>
            <article
            ...
            >
                ...
                <div className={`relative w-full ${thumbHeight} overflow-hidden bg-cream-200/60`}>
                    ...
                    <div
                        className={`file-card-fallback w-full h-full flex flex-col items-center justify-center ${isPdf ? 'hidden' : ''}`}
                    >
                        <FileText
